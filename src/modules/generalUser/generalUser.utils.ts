...
export const sendCookie = (
  ...
  res.cookie(cookieName, token, {
    ...
    maxAge: 60 * 60 * 24 * 10 * 1000, // Cookie expires in 10 day
  }); // Set the cookie
};
...
export const getStoredOTP = async (email: string): Promise<string | null> => {
  const otpRecord = await generalUser.findOne({ email });
...
export const findUserByEmail = async (
  ...
  return generalUser.findOne({ email });
};

export const findUserById = async (
  id: string
): Promise<IGeneralUser | null> => {
  return generalUser.findById(id);
};
